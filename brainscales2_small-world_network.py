...
def create_small_world_network(pop, n_neighbors=3, rewiring_prob=0.1):
    """
    Create a small-world network on the population using the Watts-Strogatz model.

    :param pop: The population of neurons.
    :param n_neighbors: Number of neighbors to connect each neuron to.
    :param rewiring_prob: Probability of rewiring each connection.
    """
    n_neurons = len(pop)
    ...
    # Randomly rewire edges with a given probability
    for i in range(n_neurons):
        for j in range(n_neighbors + 1, n_neurons // 2):
            if np.random.rand() < rewiring_prob:
                new_target = np.random.randint(n_neurons)
                connections.append((i, new_target))  # Rewired connection

    return connections

# Apply connectivity using the small-world network function
connections = create_small_world_network(pop)
for conn in connections:
    src, dest = conn  # Unpack the tuple (src, dest)
    # Ensure we are selecting PopulationView correctly
    ...
    # Projection with PopulationView
